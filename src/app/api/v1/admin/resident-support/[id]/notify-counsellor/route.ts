...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (admin.adminPermission !== "FULL_ACCESS") {
      return forbiddenError("Full access required to notify counsellor");
    }
    ...
    const notification = await prisma.residentTicketEscalation.create({
      data: {
        ticketId,
    ...
    await logAudit({
      actionType: "TICKET_NOTIFIED_TO_COUNSELLOR",
      userId: admin.userId,
    ...
    console.error("[Admin Ticket NotifyCounsellor POST]", err);
