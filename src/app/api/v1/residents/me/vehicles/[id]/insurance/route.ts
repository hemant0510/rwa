...
/** POST /api/v1/residents/me/vehicles/[id]/insurance — upload an insurance document */
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const path = `${resident.societyId}/${id}/insurance`;
    ...
    if (uploadError) {
      ...
      return internalError("Failed to upload insurance document");
    }

    // Store path in DB (not signed URL — signed URL is generated on GET)
    await prisma.vehicle.update({ where: { id }, data: { insuranceUrl: path } });
    ...
    console.error("Vehicle insurance POST error:", err);
    return internalError("Failed to upload insurance document");
  }
}
