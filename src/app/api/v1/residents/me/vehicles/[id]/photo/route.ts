...
/** POST /api/v1/residents/me/vehicles/[id]/photo — upload a vehicle photo */
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (uploadError) {
      ...
      return internalError("Failed to upload vehicle photo");
    }

    const { data: publicData } = supabaseAdmin.storage.from(PHOTO_BUCKET).getPublicUrl(path);
    const url = publicData.publicUrl;

    await prisma.vehicle.update({ where: { id }, data: { vehiclePhotoUrl: url } });

    return NextResponse.json({ url });
  } catch (err) {
    console.error("Vehicle photo POST error:", err);
    return internalError("Failed to upload vehicle photo");
  }
}
