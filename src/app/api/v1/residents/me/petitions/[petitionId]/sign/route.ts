...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (petition.status !== "PUBLISHED") {
      return NextResponse.json(
        { error: { code: "NOT_PUBLISHED", message: "Petition is not accepting changes" } },
        { status: 400 },
      );
    }
    ...
    void logAudit({
      ...
      entityType: "PetitionSignature",
      entityId: signature.id,
      oldValue: { petitionId, method: signature.method },
    });

    return NextResponse.json({ message: "Signature revoked" });
  } catch {
    return internalError("Failed to revoke signature");
  }
}
