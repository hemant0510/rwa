...
async function getResidentUser() {
  ...
  return prisma.user.findFirst({
    where,
    select: { id: true, societyId: true, idProofUrl: true },
  });
}

/** POST /api/v1/residents/me/id-proof — resident uploads their own ID proof */
export async function POST(request: NextRequest) {
  try {
    ...
    const storagePath = `${resident.societyId}/${resident.id}/id-proof.${ext}`;
    ...
    if (uploadError) {
      console.error("ID proof upload error:", uploadError.message);
      return internalError(`Storage error: ${uploadError.message}`);
    }

    await prisma.user.update({ where: { id: resident.id }, data: { idProofUrl: storagePath } });

    return NextResponse.json({ success: true, path: storagePath });
  } catch (err) {
    console.error("Resident id-proof POST error:", err);
    return internalError("Failed to upload ID proof");
  }
}

/** GET /api/v1/residents/me/id-proof — resident fetches signed URL for their ID proof */
export async function GET() {
  try {
    const resident = await getResidentUser();
    if (!resident) return unauthorizedError();

    if (!resident.idProofUrl) return NextResponse.json({ url: null });

    const supabaseAdmin = createAdminClient();
    const { data, error } = await supabaseAdmin.storage
      .from(BUCKET)
      .createSignedUrl(resident.idProofUrl, 60 * 60);

    if (error || !data?.signedUrl) return internalError("Failed to generate view link");

    return NextResponse.json({ url: data.signedUrl });
  } catch (err) {
    console.error("Resident id-proof GET error:", err);
    return internalError("Failed to get ID proof");
  }
}

/** DELETE /api/v1/residents/me/id-proof — resident removes their ID proof */
export async function DELETE() {
  try {
    ...
    const supabaseAdmin = createAdminClient();
    await supabaseAdmin.storage.from(BUCKET).remove([resident.idProofUrl]);
    await prisma.user.update({ where: { id: resident.id }, data: { idProofUrl: null } });

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("Resident id-proof DELETE error:", err);
