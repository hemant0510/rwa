...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error("[Resident Support Attachments GET]", err);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const ticket = await prisma.residentTicket.findUnique({
      where: { id, societyId: resident.societyId },
      select: { id: true, societyId: true, createdBy: true, status: true },
    });
    ...
    if (ticket.createdBy !== resident.userId) {
      return errorResponse({
        code: "FORBIDDEN",
        message: "Only ticket creator can upload attachments",
        status: 403,
      });
    }
    ...
    if (ticket.status === "CLOSED") {
      return errorResponse({
        code: "BAD_REQUEST",
        message: "Cannot upload to a closed ticket",
        status: 400,
      });
    }
    ...
    console.error("[Resident Support Attachment POST]", err);
    return internalError();
  }
}
