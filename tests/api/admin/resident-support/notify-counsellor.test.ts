...
import { POST } from "@/app/api/v1/admin/resident-support/[id]/notify-counsellor/route";
...
const makeReq = (body: unknown, id = "t-1") =>
  new Request(`http://localhost/api/v1/admin/resident-support/${id}/notify-counsellor`, {
    method: "POST",
...
describe("Admin notify-counsellor POST", () => {
  ...
  it("returns 422 on validation error", async () => {
    ...
    const res = await POST(makeReq({ reason: "x" }), makeParams());
  ...
  it("creates ADMIN_NOTIFY escalation (201)", async () => {
    mockPrisma.residentTicket.findUnique.mockResolvedValue({ id: "t-1" });
    mockPrisma.counsellorSocietyAssignment.findFirst.mockResolvedValue({ counsellorId: "c-1" });
    mockPrisma.residentTicketEscalation.findFirst.mockResolvedValue(null);
    mockPrisma.residentTicketEscalation.create.mockResolvedValue({ id: "e-10" });

    const res = await POST(makeReq({ reason: "valid notification reason text" }), makeParams());
    expect(res.status).toBe(201);
    expect(mockPrisma.residentTicketEscalation.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ source: "ADMIN_NOTIFY", counsellorId: "c-1" }),
    });
    expect(mockLogAudit).toHaveBeenCalledWith(
      expect.objectContaining({ actionType: "TICKET_NOTIFIED_TO_COUNSELLOR" }),
    );
  });
