...
import { GET } from "@/app/api/v1/societies/[id]/reports/expense-summary/route";

function makeReq(params: Record<string, string> = {}) {
  const sp = new URLSearchParams(params);
  return new NextRequest(`http://localhost/api/v1/societies/soc-1/reports/expense-summary?${sp}`);
}
...
describe("GET /api/v1/societies/[id]/reports/expense-summary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns PDF by default", async () => {
    ...
    expect(res.headers.get("Content-Disposition")).toContain("expense-summary");
  });
  ...
  it("capitalises category names correctly in excel", async () => {
    mockPrisma.expense.findMany.mockResolvedValue([mockExpenses[0]]);
    const res = await GET(makeReq({ format: "excel", session: "2025-26" }), makeParams());
    expect(res.status).toBe(200);
  });
});
