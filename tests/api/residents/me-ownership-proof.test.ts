...
import { DELETE, GET, POST } from "@/app/api/v1/residents/me/ownership-proof/route";
...
const mockResident = {
  ...
  ownershipProofUrl: null as string | null,
};

describe("POST /api/v1/residents/me/ownership-proof", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("removes old file from storage when ownershipProofUrl already set", async () => {
    const existingPath = `${SOCIETY_ID}/${RESIDENT_ID}/ownership-proof.jpg`;
    mockPrisma.user.findFirst.mockResolvedValue({
      ...mockResident,
      ownershipProofUrl: existingPath,
    });
  ...
  it("does NOT call remove when ownershipProofUrl is null", async () => {
    mockPrisma.user.findFirst.mockResolvedValue({ ...mockResident, ownershipProofUrl: null });
    const req = makePostRequest(makeFile("new.jpg", "image/jpeg"));
    await POST(req as unknown as import("next/server").NextRequest);
    expect(mockStorageBucket.remove).not.toHaveBeenCalled();
  });

  it("storage path = societyId/residentId/ownership-proof.ext", async () => {
    const req = makePostRequest(makeFile("photo.jpg", "image/jpeg"));
    const res = await POST(req as unknown as import("next/server").NextRequest);
    const body = await res.json();
    expect(body.path).toBe(`${SOCIETY_ID}/${RESIDENT_ID}/ownership-proof.jpg`);
  });

  it("updates user ownershipProofUrl with storage path", async () => {
    const req = makePostRequest(makeFile("photo.jpg", "image/jpeg"));
    await POST(req as unknown as import("next/server").NextRequest);
    expect(mockPrisma.user.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: RESIDENT_ID },
        data: { ownershipProofUrl: `${SOCIETY_ID}/${RESIDENT_ID}/ownership-proof.jpg` },
      }),
    );
  });
...
describe("GET /api/v1/residents/me/ownership-proof", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetActiveSocietyId.mockResolvedValue(null);
    mockSupabaseClient.auth.getUser.mockResolvedValue({
  ...
  it("200 with url:null when ownershipProofUrl is null", async () => {
    mockPrisma.user.findFirst.mockResolvedValue({ ...mockResident, ownershipProofUrl: null });
    const res = await GET();
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("200 with signed URL when ownershipProofUrl set", async () => {
    mockPrisma.user.findFirst.mockResolvedValue({
      ...mockResident,
      ownershipProofUrl: `${SOCIETY_ID}/${RESIDENT_ID}/ownership-proof.jpg`,
    });
    const res = await GET();
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("signed URL uses 1hr expiry (3600 seconds)", async () => {
    const path = `${SOCIETY_ID}/${RESIDENT_ID}/ownership-proof.jpg`;
    mockPrisma.user.findFirst.mockResolvedValue({ ...mockResident, ownershipProofUrl: path });
    await GET();
...
describe("DELETE /api/v1/residents/me/ownership-proof", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetActiveSocietyId.mockResolvedValue(null);
    mockSupabaseClient.auth.getUser.mockResolvedValue({
  ...
  it('403 when ownershipProofUrl is null (message: "No ownership proof on file")', async () => {
    mockPrisma.user.findFirst.mockResolvedValue({ ...mockResident, ownershipProofUrl: null });
    const res = await DELETE();
    expect(res.status).toBe(403);
    const body = await res.json();
    expect(body.error.message).toContain("No ownership proof on file");
  });
  ...
  it("sets ownershipProofUrl: null on user record", async () => {
    await DELETE();
    expect(mockPrisma.user.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: RESIDENT_ID },
...
describe("Society scoping — ownership-proof", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockSupabaseClient.auth.getUser.mockResolvedValue({
      data: { user: { id: AUTH_ID } },
