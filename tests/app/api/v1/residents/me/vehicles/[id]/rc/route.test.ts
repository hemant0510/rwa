...
function makeMultipartRequest(file: File) {
  ...
  return new Request("http://localhost/api/v1/residents/me/vehicles/veh-1/rc", {
    method: "POST",
    body: formData,
  }) as never;
}
...
describe("POST /api/v1/residents/me/vehicles/[id]/rc", () => {
  const pdfFile = new File(["rc-content"], "rc.pdf", { type: "application/pdf" });
  ...
  it("accepts image files (jpeg, png, webp)", async () => {
    const jpegFile = new File(["img"], "rc.jpg", { type: "image/jpeg" });
